...
maps::map("china", fill = FALSE, col = rainbow(20))
```
...
```{r}
groupDF <- select(china_map4, id, group) %>%
  mutate_all(as.character) %>%
    group_by(id)%>%
  summarise(group = unique(group)[1]) 

china_center <- coordinates(china_map1) %>%
  tidy

names(china_center) <- c("id", "x", "y")

china_center <- left_join(china_center, groupDF) %>%
  mutate(id = as.numeric(id)) %>%
    right_join(china_map2)
...
ggplot(china_map1, 
  ...
  geom_polygon(fill = 'beige') + # background
	geom_path(color = "grey40") +
  geom_point(aes(x ,y, group), data = china_center)
