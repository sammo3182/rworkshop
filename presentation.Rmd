...
```{r}
library(car)
outlierTest(lm_ols) # Bonferonni p-value for most extreme obs
```
...
<div class="centered">
[![present](https://espngrantland.files.wordpress.com/2014/06/9u4jd.gif)]
</div>
...
```{r eval=FALSE}
qplot(mtcars$mpg) # Equivalent to ggplot(mtcars, aes(x=mpg)) + geom_histogram()
```


* Comparing to the basic plot: 
```{r eval=FALSE}
hist(mtcars$mpg)
```


## Plotting with packages: Correlation


## Plotting with packages: Map {.columns-2}

```{r two-column, results = 'asis', echo=FALSE, out.extra = '', cache=TRUE}
...
## Two special types: `dotwhisker`{.smaller}
...
```{r eval= FALSE}
library(dplyr)
max(select(filter(mtcars, cyl == 6), mpg))
```
