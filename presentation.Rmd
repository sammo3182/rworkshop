...
* Mul<span style="color:green">Tip</span>le regression (OLS, GLS, MLM, etc.)
...
`cor.test` offers various arguments overlaping with `t.test` for more specific settings. Moreover, users can use the `method` argument to set the method to calculate the correlations, "Pearson", "Kendall", or "Spearman." (<span style="color:green">Tip</span>)
...
* we are detecting the contributions of the cylinders, housepower, and weight of the care (Xs) to the milage (Y): 
...
Of course, if you want, you can create special variables that equal `log(mpg)`, `cyl^2`, or `exp(hp)`, and use them to doe the regression.
...
# Presentationg Techniques

## Tabling
There are over twenty packages for [table presentation](http://conjugateprior.org/2013/03/r-to-latex-packages-coverage/) in R. My favorate three are `stargazer`, `xtable`, and `texreg`.

(Sorry, but all of them are for **Latex** output)

* `stargazer`: good for summary table and regular regression results
* `texreg`: when some resutls can't be presented by `stargazer`, try `texreg` (e.g., MLM results.)
...
hist(mtcars$mpg, freq = F,  # plot the dencity rather than frequency
     breaks = 10,  # change the number of bins
     col = "purple", # change the color of the bins 
     xlab="Miles Per Gallon",   # adding the x-axis lable
...
+ It breaks the plot into compoents as <span style="color:purple">scales</span> and <span style="color:purple">layers</span>---increase the flexibility.
+ To use `ggplot`, one needs to install the package `ggplot2` first.

```{r message=FALSE}
library(ggplot2)
```
+ Then, let's redo the previous histgram in `ggplot`.
...
ggplot(mtcars, aes(x=mpg)) + 
    ...
    theme_bw() + 
    ggtitle("Histgram with Normal Curve") + 
    xlab("Miles Per Gallon") + ylab("Density")
```


## Histgram comparison in `ggplot`
...
* Redo the first histgram with `qplot`:
...
Plot the compariable coefficents or other estimates (margins, predicted probabilities, etc.) with `dotwhisker`.
...
<div class="notes">
`%>%` is a piper connector embedded in `dplyr` package (which is automatically called by `dotwhisker` in the example). One can use it to avoid too complicated embedding codings:
...
# Version Control
## Just a breif introduction{.columns-2 .build}
